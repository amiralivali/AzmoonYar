...
[ApiVersion(1.0)]
public class FillInBlankItemController(IFillInBlankItemService service) : BaseController
{
    ...
    {
        var item = await service.AddFillInBlankItemsAsync(questionId,request.ToDto(),cancellationToken);
    ...
    public async Task<ApiResult<List<FillInBlankItemResponse>>> UpdateItem(long questionId,
        ...
        CancellationToken cancellationToken)
    {
        var item = await service.UpdateFillInBlankItemsAsync(questionId,request.ToDto(),cancellationToken);
    ...
    public async Task<ApiResult<List<FillInBlankAnswerResponse>>> UpdateFillInBlankAnswer(long itemId,
        ...
        CancellationToken cancellationToken)
    {
        var item = await service.UpdateFillInBlankAnswersAsync(itemId,request.ToDto(),cancellationToken);
        return item.ToResponse();
    }
